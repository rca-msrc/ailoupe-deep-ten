...
class Options():
    def __init__(self):
        ...
        # sml_eval options
        parser.add_argument('--train-n-per-class', type=int, default=None, help="Sample n number of examples per class")
        parser.add_argument('--test-n-per-class', type=int, default=None, help="Sample n number of examples 
        parser.add_argument('--mode', type=str, default='single', choices=['single', 'dual'],
                            help='Training mode: single image or dual image input')
        parser.add_argument('--plot', action='store_true', default=False, help='matplotlib')
        self.parser = parser
...
def plot_training(training_progress, save_dir=None, show=True):
    fig_loss = plt.figure(figsize=(10, 5))
    plt.plot(training_progress["train_losses"], label='Training Loss')
    plt.plot(training_progress["test_losses"], label='Validation Loss')
    plt.title('Loss Curves')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    if save_dir is not None:
        plt.savefig((save_dir / 'training_loss.png').as_posix())

    fig_accuracy = plt.figure(figsize=(10, 5))
    plt.plot(training_progress["train_accuracies"], label='Train Accuracy')
    plt.plot(training_progress["test_accuracies"], label='Test Accuracy')
    plt.plot(training_progress["test_accuracies_top5"], label='Test Accuracy (Top 5)')
    plt.title('Accuracy Curve')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()
    if save_dir is not None:
        plt.savefig((save_dir / 'training_accuracy.png').as_posix())
...
def save_eval(args, all_predictions, all_targets, all_names, all_names2=None):
    classes = args.classes

    # torcheval metrics - convert from lists to tensors
    model_outputs = torch.tensor(all_predictions)
    model_labels = torch.tensor(all_targets)

    # torcheval.metrics
    micro_accuracy = metrics.multiclass_accuracy(model_outputs, model_labels, average='micro',
                                                 num_classes=len(classes))
    macro_accuracy = metrics.multiclass_accuracy(model_outputs, model_labels, average='macro',
                                                 num_classes=len(classes))
    class_accuracy = metrics.multiclass_accuracy(model_outputs, model_labels, average=None,
                                                 num_classes=len(classes))
    class_accuracy_df = 100 * pd.DataFrame(class_accuracy.tolist(), index=classes, columns=["percent"])

    # confusion matrix
    confusion_true = metrics.multiclass_confusion_matrix(model_outputs, model_labels, num_classes=len(classes),
                                                         normalize="true")
    confusion_true_df = pd.DataFrame(confusion_true, classes, classes)

    # uses the classes array to make the header
    header = "micro (global) accuracy,macro accuracy,precision,recall,f1," + ",".join(classes)

    metric_values = [
        micro_accuracy,  # micro (global) accuracy
    ...
    metric_values.extend(class_accuracy)  # already in order of classes
    metric_strings = []
    for m in metric_values:
        if isinstance(m, np.floating):
    ...
    # save report.csv
    csv_file_with_header = save_dir / 'report.csv'
    with open(csv_file_with_header, 'w') as f:
        f.write(header + '\n')
        f.write(','.join(metric_strings))
    print(f"Report saved to {csv_file_with_header}")

    # plot and save confusions headmap
    fsize = fsize = int(10 + (len(classes) * 0.2))  # scale to num_classes
    fig = plt.figure(figsize=(fsize, fsize));
    sns.heatmap(confusion_true_df, annot=True, annot_kws={"fontsize": 8});
    confusion_path = save_dir / "confusion_true.pdf"
    fig.savefig(confusion_path.as_posix())
    print(f"confusion matrix saved to {confusion_path}")

    # save the evaluation results in a DataFrame
    if args.mode == 'single': 
        results_df = pd.DataFrame(zip(list(all_predictions), list(all_targets), list(all_names)), columns=["predictions", "targets", "names"])
    elif args.mode == 'dual':
        results_df = pd.DataFrame(zip(list(all_predictions), list(all_targets), list(all_names), list(all_names2)), columns=["predictions", "targets", "names1", "names2"])    
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # init the args
    global best_pred, acclist_train, acclist_val, training_progress
    ...
    # use imagenet style transforms -- accept default arguements otherwise
    transform_train, transform_val = encoding.transforms.get_transform(
            "imagenet", args.base_size, args.crop_size, args.rand_aug)
    ...
    # SML Datasets
    if args.eval:  # evaluation
        print(f'Evaluating dataset: {args.dataset}, mode: {args.mode}')        
        valset = get_dataset(args.dataset, args.mode, None, None, transform_val)
        ...
        print(f'Training dataset: {args.dataset}, mode: {args.mode}, n per class, train: {args.train_n_per_class}, test: {args.test_n_per_class}')
        ...
        valset = get_dataset(args.dataset, args.mode, "test", args.test_n_per_class, transform_val)
        val_sampler = torch.utils.data.distributed.DistributedSampler(valset, shuffle=False)
        val_loader = torch.utils.data.DataLoader(
            valset, batch_size=args.test_batch_size, shuffle=False,
            num_workers=args.workers, pin_memory=True,
    ...
    def train(epoch):
        ...
        losses = AverageMeter()
        top1 = AverageMeter()
        global best_pred, acclist_train, training_progress
        ...
        if args.mode == "single":
            ...
            for batch_idx, (data1, data2, target, name1, name2) in enumerate(train_loader):
                scheduler(optimizer, batch_idx, epoch, best_pred)
                if not args.mixup:
                    data1, data2, target = data1.cuda(args.gpu), data2.cuda(args.gpu), target.cuda(args.gpu)
                optimizer.zero_grad()
                ...
                if not args.mixup:
                    acc1 = accuracy(output, target, topk=(1,))
                    top1.update(acc1[0], data1.size(0))
        ...
        # record progress
        training_progress["train_accuracies"].append(top1.avg)
        training_progress["train_losses"].append(losses.avg)
    ...
    def validate(epoch):
        ...
        if args.mode == 'single':
            for batch_idx, (data, target, name) in enumerate(val_loader):
                data, target = data.cuda(args.gpu), target.cuda(args.gpu)
                with torch.no_grad():
                    output = model(data)
                    loss = criterion(output, target)
                ...
                # record predictions, targets and names
            ...
            for batch_idx, (data1, data2, target, name1, name2) in enumerate(val_loader):
                data1, data2, target = data1.cuda(args.gpu),  data2.cuda(args.gpu), target.cuda(args.gpu)
                with torch.no_grad():
                    output = model(data1, data2)
                    loss = criterion(output, target)
                    ...
                    acc1, acc5 = accuracy(output, target, topk=(1, 5))
                    top1.update(acc1[0], data1.size(0))
                    top5.update(acc5[0], data1.size(0))
    
                # record predictions, targets and names
                pred = output.data.max(1)[1]
                all_predictions.extend(pred.cpu().numpy())
                all_targets.extend(target.cpu().numpy())
                all_names.extend(name1)
                all_names2.extend(name2)
        ...
        if args.eval:
            if args.gpu == 0:
                ...
                # record evaluations
                if args.mode == 'single':
                    save_eval(args, all_predictions, all_targets, all_names)
                elif args.mode == 'dual':
                    save_eval(args, all_predictions, all_targets, all_names, all_names2)
            return
    ...
    save_dir = pathlib.Path("runs/%s/%s/%s/" % (args.dataset, args.model, args.checkname))
    if not os.path.exists(save_dir.as_posix()):
        os.makedirs(save_dir.as_posix())
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #    validate(epoch)
        validate(epoch) # validate every epoch
        ...
        if args.gpu == 0:
            ...
            if args.plot:
                plot_training(training_progress, save_dir=save_dir, show=False)
